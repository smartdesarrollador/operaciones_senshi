...
class Pedido
{
    ...
    public function getPedidosByDate($fechaPedido)
    {
        try {
            $sql = "SELECT *,clientes.DNI as clienteDni FROM clientes INNER JOIN pedidos ON clientes.idCliente = pedidos.idCliente
                    WHERE fechaPedido = '$fechaPedido' AND local_distrito = 'lince' AND deleted = 'FALSE' ORDER BY horaPedido DESC ";
 ...
 WHERE fechaEnvio= '$fechaEnvio' AND local_distrito = 'lince' AND deleted = 'FALSE'
  ORDER BY horaPedido DESC ";
    ...
    public function getPedidosLocalByFechaDeEnvio($fechaEnvio)
    {
        try {
            $sql = "SELECT * FROM pedidos
 where fechaEnvio= '$fechaEnvio' AND local_distrito = 'lince' AND deleted = 'FALSE' ORDER BY horaPedido DESC ";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    public function getPedidosLocalLimit50Pending()
    {
        try {
            $sql = "SELECT *,
                ...
                ON clientes_local.idCliente = pedidos.idClienteLocal
                INNER JOIN driver
                ON pedidos.idDriver = driver.idDriver WHERE idEstado=0 AND local_distrito = 'lince' AND deleted = 'FALSE'
    ...
    public function getPedidosLocalLimit50()
    {
        try {
            $sql = "SELECT  *,
                ...
                INNER JOIN driver
                ON pedidos.idDriver = driver.idDriver
                WHERE local_distrito = 'lince' AND deleted = 'FALSE'
                     ORDER BY idPedido DESC LIMIT 50 ";
            $lista = AccesoBD::Consultar($this->cn, $sql);
    ...
    public function getPedidosItemsByidPedido($idPedido)
    {
        try {
            $sql = "select nombreProducto,cantidad,item_descripcion,precioProducto,observacionesPaquete from pedidos INNER JOIN pedido_items ON pedidos.idPedido = pedido_items.idPedido 
                    INNER JOIN productos ON pedido_items.idProducto = productos.idProducto WHERE pedidos.idPedido = '$idPedido' AND pedidos.local_distrito = 'lince'";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
...
WHERE local_distrito = 'lince' AND deleted = 'FALSE' 
    ...
    {
        try {
            $sql = "SELECT *,clientes.DNI as clienteDni FROM clientes INNER JOIN pedidos ON clientes.idCliente = pedidos.idCliente WHERE idEstado=0 AND local_distrito = 'lince' AND deleted = 'FALSE'
                     ORDER BY idPedido DESC LIMIT 50 ";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
    ...
    {
        try {
            $sql = "SELECT * FROM clientes INNER JOIN pedidos ON clientes.idCliente = pedidos.idCliente where local_distrito = 'lince' AND deleted = 'FALSE'";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    public function changeFeedBackStatus($idPedido, $status, $token)
    {
        $sql = "UPDATE pedidos set
                feedBackEnviado= '$status',feedBackToken='$token' where local_distrito = 'lince' AND idPedido = '$idPedido'";
    ...
    {
        try {
            $sql = "SELECT count(idPedido) as total FROM pedidos WHERE pedidos.local_distrito = 'lince' AND pedidos.idCliente > 0  AND deleted = 'FALSE'";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista[0];
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    {
        $sql = "UPDATE pedidos set
                idEstado= '$idEstado' where idPedido = '$idPedido' AND local_distrito = 'lince'";
        $id = AccesoBD::Insertar($this->cn, $sql);
        return $id;
    }
 ...
 WHERE idPedido = '$idPedido' AND local_distrito = 'lince' ORDER BY horaPedido DESC ";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista[0];
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
...
INNER JOIN driver ON pedidos.idDriver = driver.idDriver
 WHERE idPedido = '$idPedido' AND local_distrito = 'lince' ORDER BY horaPedido DESC ";
    ...
    public function addPedidoLocal($direccionPedido,
                                   $pedidoTelefono,
                                   $pedidoObservaciones, $precioTotal, $delivery = 'true',
                                   ...
                                   $driverLocal,$costoEnvioLocal,$adicionalEnvioLocal,$idDriver, $costoEnvioPagado)
    {
        ...
        $sql = "INSERT INTO pedidos ( direccionPedido,
                ...
                costoEnvioPagado
                     ...
                     '$dateTime','$delivery','$referencia','$distrito','$documento',
    ...
    public function getPedidosByDriver($idDriver)
    {
        try {
            $sql = "SELECT * FROM pedidos INNER JOIN estadopedido 
                    ...
                    where idDriver = '$idDriver' AND local_distrito = 'lince' AND deleted = 'FALSE' AND LENGTH(idCliente)>0 ORDER BY fechaPedido DESC";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    public function getPedidosByDriverAndFechaPedido($idDriver, $fechaPedido)
    {
        try {
            ...
            AND fechaPedido = '$fechaPedido' AND local_distrito = 'lince' AND deleted = 'FALSE' AND LENGTH(idCliente)>0 ORDER BY fechaPedido DESC";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    public function getPedidosByDriverLocal($idDriver)
    {
        try {
            $sql = "SELECT * FROM pedidos INNER JOIN estadopedido 
                    ON pedidos.idEstado = estadopedido.idEstado 
                    where idDriver = '$idDriver' AND local_distrito = 'lince' AND deleted = 'FALSE' AND LENGTH(idClienteLocal)>0 ORDER BY fechaPedido DESC";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    public function getPedidosByDriverAndFechaPedidoLocal($idDriver, $fechaPedido)
    {
        try {
            $sql = "SELECT * FROM pedidos INNER JOIN estadopedido 
            ON pedidos.idEstado = estadopedido.idEstado where idDriver = '$idDriver' 
            AND fechaPedido = '$fechaPedido' AND local_distrito = 'lince' AND LENGTH(idClienteLocal)>0 AND deleted = 'FALSE' ORDER BY fechaPedido DESC";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
...
INNER JOIN driver ON pedidos.idDriver = driver.idDriver WHERE fechaEnvio= '$fechaPedido' AND local_distrito = 'lince' AND pedidos.idDriver != 3 AND deleted = 'FALSE'";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    public function reporteVentasUltimos6Meses()
    {
        try {
            ...
            $sql = "SELECT SUM(precioTotal) as montoVentas,MONTHNAME(fechaPedido) as mes FROM pedidos WHERE local_distrito = 'lince' AND  CHAR_LENGTH(idCliente)>0 GROUP BY MONTHNAME(fechaPedido) ORDER BY fechaPedido ASC";
    ...
    public function generarExcelPrimerReporte($monthAndYear)
    {
        try {
            ...
            $sql = "SELECT * FROM pedidos WHERE local_distrito = 'lince' AND MONTH(fechaPedido) = '$month' AND YEAR(fechaPedido) = '$year' AND CHAR_LENGTH(idCliente)>0";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
    ...
    {
        try {
            $sql = "SELECT * FROM `pedidos` WHERE local_distrito = 'lince' AND deleted= 'FALSE' AND fechaPedido BETWEEN '$fechaInicio' AND '$fechaFinal'";
            $lista = AccesoBD::Consultar($this->cn, $sql);
            return $lista;
        } catch (Exception $e) {
            $mensaje = "Fecha: " . date("Y-m-d H:i:s") . "\n" .
